...
static void batch_exec( vector<stCoClosure_t*> &v )
{
	...
	{
		pthread_t tid;
		pthread_create( &tid,0,thread_func,v[i] );  // 执行
		ths.push_back( tid );
	}
	for( size_t i=0;i<v.size();i++ )
	{
...
int main( int argc,char *argv[] )
{
	...
	int total = 100;
	vector<int> v2;
	co_ref( ref,total,v2,m);  // 将total, v2, m 作为成员添加到 ref，注意这个ref不需要定义。
	for(int i=0;i<10;i++)
	{
		co_func( f,ref,i )  // 定义一个名为f的闭包函数， 参数是 ref 和 i
		{
		...
		co_func_end;    // 闭包函数结束标志
		v.push_back( new f( ref,i ) );  // 闭包函数底层其实是个对象
	}
	for(int i=0;i<2;i++)
	{
		co_func( f2,i )
